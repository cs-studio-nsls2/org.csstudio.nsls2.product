...
mvn -Dtycho.mode=maven org.eclipse.tycho:tycho-versions-plugin:1.0.0:set-version -DnewVersion=$VERSION -Dartifacts=nsls2-product,products-csstudio-nsls2-features,org.csstudio.nsls2.product.feature,nsls2-repository
...
then
  ...
  git tag CS-Studio-$VERSION
  echo ::: Pushing changes :::
  git push origin
  git push origin CS-Studio-$VERSION
fi
